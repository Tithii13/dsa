...
// in new node link
newNode->next = *head;// put new node address into last
// node link(next)
...
struct Node* current = head;
int count = 0;
// if list is empty simply return length zeroif (head == NULL) {
return 0;
}
// traverse first to last node
else {
...
// traverse second to firstwhile (previous->next != *head) {
...
// if yes then delete it and returnif (current->next == current) {
*head = NULL;
return;
}
...
int main()
{
struct Node* head = NULL;Insert(&head, 99);
Insert(&head, 11);
Insert(&head, 22);
Insert(&head, 33);
Insert(&head, 44);
